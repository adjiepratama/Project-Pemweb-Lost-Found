...
<body class="bg-gray-50 text-gray-800">
                    ...
                    <a href="{{ route('user.profile') }}" class="flex items-center gap-3 border-l pl-6 border-gray-200 group">
                        ...
                        <div class="h-9 w-9 rounded-full bg-gray-200 overflow-hidden border border-gray-300 ring-2 ring-gray-100 group-hover:ring-cyan-200 transition-all">
                            @if(Auth::user()->profile_photo)
                                <img src="{{ \Illuminate\Support\Str::startsWith(Auth::user()->profile_photo, 'http') ? Auth::user()->profile_photo : asset('storage/' . Auth::user()->profile_photo) }}" class="h-full w-full object-cover">
                            @else
               ...
               <form action="{{ route('user.dashboard') }}" method="GET">
                    <section class="mb-8 border border-gray-100 rounded-xl p-6 shadow-sm bg-white">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                            <select name="category" class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                                ...
                                <option value="Perabotan" {{ request('category') == 'Perabotan' ? 'selected' : '' }}>Perabotan</option>
                            ...
                            <select name="location" class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                                ...
                                <option value="Gedung Griya" {{ request('location') == 'Gedung Griya' ? 'selected' : '' }}>Gedung Griya</option>
                                <option value="Gedung Modular" {{ request('location') == 'Gedung Modular' ? 'selected' : '' }}>Gedung Modular</option>
                                <option value="Gedung Gor Abc" {{ request('location') == 'Gedung Gor Abc' ? 'selected' : '' }}>Gedung Gor Abc</option>
                                <option value="Selasar" {{ request('location') == 'Selasar' ? 'selected' : '' }}>Selasar</option>
                                <option value="Mushola" {{ request('location') == 'Mushola' ? 'selected' : '' }}>Mushola</option>
                                <option value="Kantin Atas" {{ request('location') == 'Kantin Atas' ? 'selected' : '' }}>Kantin Atas</option>
                                <option value="Kantin Bawah" {{ request('location') == 'Kantin Bawah' ? 'selected' : '' }}>Kantin Bawah</option>
                            </select>
                            <select name="status" class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                                ...
                                <option value="Didonasikan" {{ request('status') == 'Didonasikan' ? 'selected' : '' }}>Didonasikan</option>
                            </select>
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        {{-- LOGIKA GAMBAR PINTAR (PHP di dalam View) --}}
                        @php
                            ...
                            if ($item->image) {
                                if (\Illuminate\Support\Str::startsWith($item->image, 'http')) {
                                    ...
                                    $finalImage = asset('storage/' . $item->image);
                                } else {
                                    $finalImage = asset($item->image);
                                }
                            }
                        @endphp

                        <article class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
                            <div class="h-56 bg-gray-100 w-full overflow-hidden relative">
                                <img src="{{ $finalImage }}" alt="{{ $item->title }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            </div>
                            <div class="p-4">
                                ...
                                <div class="mt-4 w-full">
                                    ...
                                    class="w-full py-2 bg-gray-50 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-100 transition-colors">
                                    Lihat Detail
                                    </button>
                                </div>
                            </div>
            ...
            <div class="p-6">
                <div class="flex flex-col md:flex-row gap-6">
                    <div class="w-full md:w-1/2">
                        <div class="h-80 bg-gray-100 rounded-lg overflow-hidden border border-gray-200 flex items-center justify-center">
                            <img id="modalImage" src="" alt="Detail Gambar" class="w-full h-full object-contain">
                        </div>
                        <div class="flex gap-2 mt-4 hidden"> <div class="w-16 h-16 border-2 border-cyan-400 rounded-lg overflow-hidden p-1">
                    ...
                    <div class="w-full md:w-1/2 space-y-4">
                        ...
                        <div class="pt-4">
                            <button onclick="openClaimModal()" 
                                    id="btnAjukanKlaim"
                                    class="w-full bg-cyan-400 hover:bg-cyan-500 text-white font-bold py-3 px-4 rounded-lg shadow-md transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed">
                                Ajukan Klaim Kepemilikan
                            </button>
                            <p id="donatedMessage" class="hidden text-xs text-center mt-2 font-semibold">
                                </p>
                        </div>
                    </div>
                </div>
    ...
    <div id="claimModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-[60]">
        ...
        <div>
        <label class="block text-gray-700 text-sm font-bold mb-2">No Induk <span class="text-red-500">*</span></label>
    ...
    <script>
        ...
        function openDetailModal(button) {
            ...
            // --- UPDATE LOGIKA KLAIM (REVISI) ---
            const btnKlaim = document.getElementById('btnAjukanKlaim');
            const msgDonated = document.getElementById('donatedMessage');

            // Reset state tombol ke default (aktif)
            btnKlaim.disabled = false;
            btnKlaim.innerText = "Ajukan Klaim Kepemilikan";
            btnKlaim.classList.remove('bg-gray-300', 'cursor-not-allowed');
            btnKlaim.classList.add('bg-cyan-400', 'hover:bg-cyan-500');
            
            // Sembunyikan pesan & reset style pesan
            msgDonated.classList.add('hidden');
            msgDonated.classList.remove('text-orange-600', 'text-purple-600');

            if (status === 'donated') {
                // KASUS 1: DONATED (MATI TOTAL)
                btnKlaim.disabled = true;
                btnKlaim.innerText = "Barang Sudah Didonasikan";
                btnKlaim.classList.add('bg-gray-300', 'cursor-not-allowed');
                btnKlaim.classList.remove('bg-cyan-400', 'hover:bg-cyan-500');
                
                msgDonated.innerText = "Maaf, barang ini sudah didonasikan dan tidak bisa diklaim lagi.";
                msgDonated.classList.add('text-purple-600');
                msgDonated.classList.remove('hidden');

            } else if (status === 'returned') {
                // KASUS 2: RETURNED (TETAP BISA KLAIM, TAPI ADA WARNING)
                // Tombol tetap aktif, tidak di-disable
                
                msgDonated.innerText = "Catatan: Barang ini tercatat sudah dikembalikan. Jika ini kesalahan, silakan ajukan klaim ulang.";
                msgDonated.classList.add('text-orange-600'); // Warna peringatan (bukan error)
                msgDonated.classList.remove('hidden');
            }
